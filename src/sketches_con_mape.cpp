...
EvalResult evaluate_countsketch(const vector<pair<string,uint64_t>> &counts, uint64_t N,
    ...
    for (const auto &p : counts) {
        ...
        uint64_t mape_err=aerr/((uint64_t)llabs((int64_t)truec)); //
        abs_errs.push_back(aerr);
        sum_abs += (double)aerr;
        sum_mape+=(double)mape_err;
    ...
    double mape = counts.empty() ? 0.0 : 100.0*sum_mape/(double)counts.size();
...
int main(int argc, char **argv) {
    ...
    ofstream fout(out);
    fout << "method,params,mem_bytes,mean_abs_err,median_abs_err,max_abs_err,precision,recall,f1,tp,fp,fn,mape\n";
